...
class RowRenderer {
    ...
    removePlaceholders() {
        const tempTrs = this.table.tbody.querySelectorAll('[role=temp]')
    ...
    #renderRow(data, columns) {
        ...
        for (const column of columns) {
            const td = column.isSpecialColumn() ?
                this.#renderSpecialColumnCell(column) :
    ...
    #renderCell(column, data) {
        ...
        if (!column.content) {
            ...
            column.formatValue(span, value, this.table.format)
     ...
     * @param { Column } column
     */
    #renderSpecialColumnCell(column) {
        const td = document.createElement('td')

        if (column.specialColumn === SpecialColumnKind.Expand) {
            td.innerHTML = '<button role="expand"></button>'
        }

        if (column.specialColumn === SpecialColumnKind.Dropdown) {
