...
class ColumnEdit {
    ...
    unpin(column) {
        ...
        this.#reorderColumns(column, column.index)
    }
    ...
    remove(column) {
        if (column.hasStackedParent()) {
            const parent = this.table.getColumn(column.parentIndex)
            ...
            if (parent.stacked.length === 1) {
                this.table.columns[column.parentIndex] = parent.stacked[0]
                parent.stacked[0].parentIndex = null
            }
