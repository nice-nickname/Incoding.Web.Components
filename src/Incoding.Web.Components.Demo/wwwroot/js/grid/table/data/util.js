...
class AggregateUtil {
    ...
    static executeFormula(data, formula) {
        ...
        for (const { variable, origin } of entries) {
            const value = executed[variable]
            formula = formula.replace(origin, value)
        }
    ...
    static parseFormula(formula) {
        const regex = /(SUM|AVG|MIN|MAX|COUNT)\((.*?)\)/gm
        const operationRegex = /(SUM|AVG|MIN|MAX|COUNT)/gm

        const result = []

        for (const operation of formula.match(regex)) {
            const operator = operation.match(operationRegex)[0].toLowerCase()
            const variable = operation.substring(
                operation.indexOf('(') + 1,
                operation.indexOf(')'),
            )

            result.push({ operator, variable, origin: operation })
        }
